...
if (ONEAD.oid_call === undefined) {
    ONEAD.oid_call = function (_params) {
        ...
        if (!_1stCookie) {
            // 1st cookie 不存在，就直接寫入
            ONEAD.set_cookie('oid', _cookieValue);
        } else if (_cookieValue.ts > 0 && _cookieValue.ts < _1stCookie.ts) {
            //如果存在，就比 ts 看誰舊
            ONEAD.set_cookie('oid', _cookieValue);
        }
        if (_params.dnt === 0) {
...
var getVastParseResult = function (vastUrl, param, callbackAfterCheckVastResponse, sourceUrl) {
    ...
    if (sourceUrl) {
        (function () {
            var sendReportWithCookieSources = ['aralego.com'];
    ...
    vastObj.client.askForVastResponse({ vast_url: vastUrl, asset_url: window.location.href }, function (error, result) {
        ...
        if (ONEAD && ONEAD.wrapper) {
            ...
            if (doc.getElementById(ONEAD.wrapper)) {
                ...
                if (ONEAD.wrapper.indexOf(ONEAD_CONST.PLAY_MODE.IC) > -1) {
                    ...
                    if (param.player_type === ONEAD_CONST.PLAYER_TYPE.H5_970X250 || param.player_type === ONEAD_CONST.PLAYER_TYPE.H5_640X480) {
                        ONEAD.ONEAD_css(ONEAD.wrapper, doc, {
                            height: '0px',
                            padding: '0px',
                        });
        ...
        if (_is_vast) {
            ...
            if (ONEAD.gotVAST) {
                ...
                if (checkIfUseVpaid(vastResult)) {
                    ...
                    _.Q.ALL([_.get_script(HTML5_PLAYER_SRC, node), _.get_script(_vpaidSrc, node)], callbackGotVpaidCode);
                } else {
                    _.Q.ALL([_.get_script(HTML5_PLAYER_SRC, node)], set_vast_info);
                }
            }
...
ONEAD.ONEAD_mobile_init_player_ima = function (params, callbackIfUrlIsUseless) {
    ...
    imaScript.onload = function () {
        //@zone FRON-1133
        _.Q.ALL([_.get_script(pv2_src, node)], startInitPlayer);
    };
...
if (window.ONEAD_rejoin_response_freq === undefined) {
    window.ONEAD_rejoin_response_freq = function (freq_multiple) {
        ...
        if (typeof freq_multiple !== 'undefined') {
            ...
            if (window.ONEAD_is_mobile()) {
                const_modes = [ONEAD_CONST.PLAY_MODE.MIF, ONEAD_CONST.PLAY_MODE.MIC, ONEAD_CONST.PLAY_MODE.MIR, ONEAD_CONST.PLAY_MODE.MAIR, ONEAD_CONST.PLAY_MODE.MIT];
            }
...
(function (ONEAD) {
    ...
    staging.css = function (styles) {
        for (var s in styles) {
            staging.style[s] = styles[s];
        }
    };
    ...
    staging.innerHTML = 'ONEAD STAGING, UID: ' + ONEAD.uid;
